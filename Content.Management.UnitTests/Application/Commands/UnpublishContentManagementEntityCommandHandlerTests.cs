...
public class UnpublishContentManagementEntityCommandHandlerTests
{
    private readonly IContentManagementEntityRepository _repository = Substitute.For<IContentManagementEntityRepository>();
    private readonly UnpublishContentManagementEntityValidator _validator = new();

    private UnpublishContentManagementEntityCommandHandler CreateHandler() =>
        new(NullLogger<UnpublishContentManagementEntityCommandHandler>.Instance, _repository, _validator);

    [Fact]
    public async Task Handle_WhenEntityNotFound_CreatesUnpublishedEntity()
    {
        ...
        var result = await CreateHandler().Handle(
            new UnpublishContentManagementEntityCommand("id-1", "v2", 2),
            CancellationToken.None);

        result.Should().BeTrue();
        await _repository.Received(1).AddAsync(
            Arg.Is<ContentManagementEntity>(e => !e.IsPublished && e.Version == 2),
            Arg.Any<CancellationToken>());
    }

    [Fact]
    public async Task Handle_WhenEntityExists_UnpublishesAndReturnsTrue()
    {
        ...
        var result = await CreateHandler().Handle(
            new UnpublishContentManagementEntityCommand("id-1", "v1", 1),
            CancellationToken.None);
    ...
    public async Task Handle_WhenCommandIsInvalid_ThrowsValidationException()
    {
        Func<Task> act = () => CreateHandler().Handle(
            new UnpublishContentManagementEntityCommand("", "", 0),
            CancellationToken.None);
